...
calculate_alpha<-function(otu){
  ...
  data_pielou<-data_shannon/log(data_shannon,base = exp(1))
...
4. 利用函数calculate_alpha()，创建绘图所需数据框并重命名其中的列：
```{r}
...
6. 现在，需要绘制分面箱线图，以在同一张图片中表示三种不同的alpha多样性，为此，我们需要将plot_df的**宽表**转化为**长表**：
```{r}
#宽表转化为长表
